				...
				<span class="message-time"> -
					<?php if ( $reply_messages->have_posts() ) { ?>
						<?php foreach ($reply_messages->posts as $rpost) {?>
								<?php echo sprintf(__('%s', 'workup'), human_time_diff(get_the_time('U', $rpost), current_time('timestamp')) ); ?>
						<?php } ?>
					<?php } else { ?>
							<?php echo sprintf(__('%s', 'workup'), human_time_diff(get_the_time('U', $post), current_time('timestamp')) ); ?>
					<?php } ?>
				</span>
			</h4>
