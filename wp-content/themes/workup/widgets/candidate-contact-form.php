...
?>
	...
	<div class="contact-form-candidate in-sidebar">
	    <form method="post" action="?" class="contact-form-wrapper">
	    	<div class="row">
		        <div class="col-sm-12">
			        <div class="form-group">
			            <input type="text" class="form-control" name="subject" placeholder="<?php esc_attr_e( 'Subject', 'workup' ); ?>" required="required">
			        </div><!-- /.form-group -->
			    </div>
			    <div class="col-sm-12">
			        <div class="form-group">
			            <input type="email" class="form-control" name="email" placeholder="<?php esc_attr_e( 'E-mail', 'workup' ); ?>" required="required" value="<?php echo esc_attr($email); ?>">
			        </div><!-- /.form-group -->
			    </div>
			    <div class="col-sm-12">
			        <div class="form-group">
	        ...
	        <div class="form-group">
	            <textarea class="form-control" name="message" placeholder="<?php esc_attr_e( 'Message', 'workup' ); ?>" required="required"></textarea>
	        </div><!-- /.form-group -->
	        ...
	        <button class="button btn btn-theme btn-block" name="contact-form"><?php echo esc_html__( 'Send Now', 'workup' ); ?><i class="next flaticon-right-arrow"></i></button>
	    </form>
...
if ( class_exists('WP_Job_Board_Wc_Paid_Listings_Contact_Package') && method_exists('WP_Job_Board_Wc_Paid_Listings_Contact_Package', 'check_user_can_contact_candidate') ) {
	...
	if ( !WP_Job_Board_Wc_Paid_Listings_Contact_Package::check_user_can_contact_candidate($post) ) {
		...
		<div class="alert alert-warning">
			<?php echo sprintf(__('You have no package. To contact "%s" you need <a href="%s" class="text-theme">click here</a> to subscribe a package.', 'workup'), $post->post_title, $package_page_url); ?>
		</div>
		<?php
	}
