...
if ( ! function_exists( 'workup_body_classes' ) ) {
	function workup_body_classes( $classes ) {
		global $post;
		$show_footer_mobile = workup_get_config('show_footer_mobile', true);
		...
		if ( workup_get_config('preload', true) ) {
			$classes[] = 'apus-body-loading';
		}
		if ( workup_get_config('image_lazy_loading') ) {
		...
		if ( workup_get_config('keep_header') ) {
			$classes[] = 'has-header-sticky';
		}
		
		return $classes;
	}
	add_filter( 'body_class', 'workup_body_classes' );
}

if ( ! function_exists( 'workup_get_shortcode_regex' ) ) {
	function workup_get_shortcode_regex( $tagregexp = '' ) {
...
if ( ! function_exists( 'workup_tagregexp' ) ) {
	function workup_tagregexp() {
		return apply_filters( 'workup_custom_tagregexp', 'video|audio|playlist|video-playlist|embed|workup_media' );
	}
}

if ( !function_exists('workup_get_header_layouts') ) {
	function workup_get_header_layouts() {
		$headers = array();
		$args = array(
			'posts_per_page'   => -1,
			'offset'           => 0,
...
if ( !function_exists('workup_get_header_layout') ) {
	function workup_get_header_layout() {
		global $post;
		if ( is_page() && is_object($post) && isset($post->ID) ) {
			...
			if ( empty($header) || $header == 'global' ) {
				return workup_get_config('header_type');
			}
			return $header;
		}
		return workup_get_config('header_type');
	}
	add_filter( 'workup_get_header_layout', 'workup_get_header_layout' );
}

function workup_display_header_builder($header_slug) {
	$args = array(
		'name'        => $header_slug,
		'post_type'   => 'apus_header',
		'post_status' => 'publish',
	...
	foreach ( $posts as $post ) {
		...
		if ( workup_get_config('keep_header') ) {
			...
			echo apply_filters( 'workup_generate_post_builder', do_shortcode( $post->post_content ), $post, $post->ID);
		if ( workup_get_config('keep_header') ) {
			echo '</div>';
	        echo '</div>';
	    }
...
if ( !function_exists('workup_get_footer_layouts') ) {
	function workup_get_footer_layouts() {
		$footers = array();
		$args = array(
			'posts_per_page'   => -1,
			'offset'           => 0,
...
if ( !function_exists('workup_get_footer_layout') ) {
	function workup_get_footer_layout() {
		if ( is_page() ) {
			global $post;
			$footer = '';
			if ( is_object($post) && isset($post->ID) ) {
				...
				if ( empty($footer) || $footer == 'global' ) {
					return workup_get_config('footer_type', '');
				}
			}
			return $footer;
		}
	...
	add_filter('workup_get_footer_layout', 'workup_get_footer_layout');
}

function workup_display_footer_builder($footer_slug) {
	$show_footer_desktop_mobile = workup_get_config('show_footer_desktop_mobile', false);
	$args = array(
		'name'        => $footer_slug,
		'post_type'   => 'apus_footer',
		'post_status' => 'publish',
...
if ( !function_exists('workup_blog_content_class') ) {
	function workup_blog_content_class( $class ) {
		$page = 'archive';
		if ( is_singular( 'post' ) ) {
            $page = 'single';
        }
		if ( workup_get_config('blog_'.$page.'_fullwidth') ) {
			return 'container-fluid';
		}
		return $class;
	}
}
add_filter( 'workup_blog_content_class', 'workup_blog_content_class', 1 , 1  );


if ( !function_exists('workup_get_blog_layout_configs') ) {
	function workup_get_blog_layout_configs() {
		$page = 'archive';
		if ( is_singular( 'post' ) ) {
            $page = 'single';
        }
		$left = workup_get_config('blog_'.$page.'_left_sidebar');
		$right = workup_get_config('blog_'.$page.'_right_sidebar');

		switch ( workup_get_config('blog_'.$page.'_layout') ) {
		 	case 'left-main':
...
if ( !function_exists('workup_page_content_class') ) {
	function workup_page_content_class( $class ) {
		global $post;
		if (is_object($post)) {
...
add_filter( 'workup_page_content_class', 'workup_page_content_class', 1 , 1  );

if ( !function_exists('workup_get_page_layout_configs') ) {
	function workup_get_page_layout_configs() {
		global $post;
		if ( is_object($post) ) {
...
if ( ! function_exists( 'workup_get_first_url_from_string' ) ) {
	function workup_get_first_url_from_string( $string ) {
...
if ( !function_exists( 'workup_get_link_attributes' ) ) {
	function workup_get_link_attributes( $string ) {
...
if ( !function_exists( 'workup_post_media' ) ) {
	function workup_post_media( $content ) {
		$is_video = ( get_post_format() == 'video' ) ? true : false;
		$media = workup_get_first_url_from_string( $content );
		$media = $media['link'];
		if ( ! empty( $media ) ) {
			global $wp_embed;
			$content = do_shortcode( $wp_embed->run_shortcode( '[embed]' . $media . '[/embed]' ) );
		} else {
			$pattern = workup_get_shortcode_regex( workup_tagregexp() );
...
if ( !function_exists( 'workup_post_gallery' ) ) {
	function workup_post_gallery( $content, $args = array() ) {
		$output = '';
		$defaults = array( 'size' => 'large' );
		$args = wp_parse_args( $args, $defaults );
	    $gallery_filter = workup_gallery_from_content( $content );
...
if (!function_exists('workup_gallery_from_content')) {
    function workup_gallery_from_content($content) {
...
if ( !function_exists( 'workup_random_key' ) ) {
    function workup_random_key($length = 5) {
...
if ( !function_exists('workup_substring') ) {
    function workup_substring($string, $limit, $afterlimit = '[...]') {
        if ( empty($string) ) {
        	return $string;
        }
...
function workup_is_apus_framework_activated() {
	return defined('APUS_FRAMEWORK_VERSION') ? true : false;
}

function workup_is_cmb2_activated() {
	return defined('CMB2_LOADED') ? true : false;
}

function workup_is_woocommerce_activated() {
	return class_exists( 'woocommerce' ) ? true : false;
}

function workup_is_revslider_activated() {
	return class_exists( 'RevSlider' ) ? true : false;
}

function workup_is_mailchimp_activated() {
	return class_exists( 'MC4WP_Form_Manager' ) ? true : false;
}

function workup_is_wp_job_board_activated() {
	return class_exists( 'WP_Job_Board' ) ? true : false;
}

function workup_is_wp_job_board_wc_paid_listings_activated() {
	return class_exists( 'WP_Job_Board_Wc_Paid_Listings' ) ? true : false;
}

function workup_is_wp_private_message() {
	return class_exists( 'WP_Private_Message' ) ? true : false;
}


function workup_header_footer_templates( $template ) {
...
add_filter( 'template_include', 'workup_header_footer_templates' );