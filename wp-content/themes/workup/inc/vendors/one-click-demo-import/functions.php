...
function workup_ocdi_import_files() {
    $demos = array();
    for ($i=1; $i <= 5; $i++) {
        $demos[] = array(
            ...
            'local_import_redux'           => array(
                array(
                    'file_path'   => trailingslashit( get_template_directory() ) . 'inc/vendors/one-click-demo-import/default/redux-options.json',
                    'option_name' => 'workup_theme_options',
                ),
            ),
            'import_preview_image_url'     => trailingslashit( get_template_directory_uri() ) . 'inc/vendors/one-click-demo-import/default/home'.$i.'.jpg',
            'import_notice'                => esc_html__( 'Import process may take 5-10 minutes. If you facing any issues please contact our support.', 'workup' ),
            'preview_url'                  => 'https://apusthemes.com/wp-demo/workup/',
        );
    }
    return apply_filters( 'workup_ocdi_files_args', $demos );
}
add_filter( 'pt-ocdi/import_files', 'workup_ocdi_import_files' );

function workup_ocdi_after_import_setup( $selected_import ) {
    ...
    if ( file_exists($file) ) {
        workup_ocdi_import_settings($file);
    }

}
add_action( 'pt-ocdi/after_import', 'workup_ocdi_after_import_setup' );

function workup_ocdi_import_settings($file) {
    ...
    if ( !empty($datas['page_options']) ) {
        workup_ocdi_import_page_options($datas['page_options']);
    }
    if ( !empty($datas['metadata']) ) {
        workup_ocdi_import_some_metadatas($datas['metadata']);
    }
}

function workup_ocdi_import_page_options($datas) {
    if ( $datas ) {
...
function workup_ocdi_import_some_metadatas($datas) {
    if ( $datas ) {
...
function workup_ocdi_before_widgets_import() {
...
add_action( 'pt-ocdi/before_widgets_import', 'workup_ocdi_before_widgets_import' );