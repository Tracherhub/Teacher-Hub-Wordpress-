...
<div class="job-submission-preview-form-wrapper">
	...
	<form action="<?php echo esc_url($form_obj->get_form_action());?>" class="cmb-form" method="post" enctype="multipart/form-data" encoding="multipart/form-data">
		...
		<div class="action-preview">
			<button class="button btn btn-success" name="continue-submit-job"><?php esc_html_e('Submit Job', 'workup'); ?></button>
			<button class="button btn btn-danger" name="continue-edit-job"><?php esc_html_e('Edit Job', 'workup'); ?></button>
		</div>
		<?php
			if ( $job_layout !== 'v1' ) {
