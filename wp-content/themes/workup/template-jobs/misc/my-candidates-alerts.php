...
<div class="box-dashboard-wrapper">
	...
	<h3 class="widget-title"><?php echo esc_html__('Candidate Alerts','workup') ?></h3>
	...
	if ( !empty($alerts) && !empty($alerts->posts) ) {
		...
		<div class="table-responsive">
			<table class="job-table">
				<thead>
					<tr>
						<th class="job-title"><?php esc_html_e('Title', 'workup'); ?></th>
						<th class="alert-query"><?php esc_html_e('Alert Query', 'workup'); ?></th>
						<th class="job-number"><?php esc_html_e('Number Jobs', 'workup'); ?></th>
						<th class="job-times"><?php esc_html_e('Times', 'workup'); ?></th>
				...
				<tbody>
					...
					foreach ($alerts->posts as $alert_id) {
						...
						<tr <?php post_class('job-alert-wrapper'); ?>>
						    ...
						    <td>
						        <div class="job-found">
						            <?php echo sprintf(esc_html__('Jobs found %d', 'workup'), intval($count_jobs) ); ?>
						        </div>
					        </td>
					        <td>
						        <div class="job-metas">
						    ...
						    <td>
								<a href="javascript:void(0)" class="btn-remove-job-alert" data-alert_id="<?php echo esc_attr($alert_id); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce( 'wp-job-board-remove-candidate-alert-nonce' )); ?>"><?php esc_html_e('Remove', 'workup'); ?></a>
							</td>
						</tr><!-- #post-## -->
	...
	<?php } else { ?>
		<div class="not-found"><?php esc_html_e('No candidate alert found.', 'workup'); ?></div>
	<?php } ?>
	</div>
</div>