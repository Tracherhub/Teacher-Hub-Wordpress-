...
class WP_Job_Board_Wc_Paid_Listings_Mixes {
	...
	public static function create_user_package( $user_id, $product_id, $order_id ) {
		...
		$args = apply_filters( 'wp_job_board_wc_paid_listings_create_user_package_data', array(
		...
		if ( $user_package_id ) {
			...
			do_action('wp_job_board_wc_paid_listings_create_user_package_meta', $user_package_id, $user_id, $product_id, $order_id);
		}
	...
	public static function approve_job_with_package( $job_id, $user_id, $user_package_id ) {
		if ( self::package_is_valid( $user_id, $user_package_id ) ) {
			...
			if ( $post_type === 'job_listing' ) {
				delete_post_meta( $job_id, WP_JOB_BOARD_JOB_LISTING_PREFIX.'expiry_date' );
			...
			do_action('wp_job_board_wc_paid_listings_approve_job_with_package', $job_id, $user_id, $user_package_id);
		}
	}
	...
	public static function create_user_cv_package( $user_id, $product_id, $order_id ) {
		...
		if ( !$package->is_type( 'cv_package' ) ) {
			return false;
		}

		$args = apply_filters( 'wp_job_board_wc_paid_listings_create_user_cv_package_data', array(
			'post_title' => $package->get_title(),
			'post_status' => 'publish',
			'post_type' => 'job_package',
		), $user_id, $product_id, $order_id);
		...
		if ( $user_package_id ) {
			...
			do_action('wp_job_board_wc_paid_listings_create_user_cv_package_meta', $user_package_id, $user_id, $product_id, $order_id);
		}
	...
	public static function create_user_contact_package( $user_id, $product_id, $order_id ) {
		...
		if ( !$package->is_type( 'contact_package' ) ) {
			return false;
		}

		$args = apply_filters( 'wp_job_board_wc_paid_listings_create_user_contact_package_data', array(
			'post_title' => $package->get_title(),
			'post_status' => 'publish',
			'post_type' => 'job_package',
		), $user_id, $product_id, $order_id);
		...
		if ( $user_package_id ) {
			...
			do_action('wp_job_board_wc_paid_listings_create_user_contact_package_meta', $user_package_id, $user_id, $product_id, $order_id);
		}
	...
	public static function create_user_candidate_package( $user_id, $product_id, $order_id ) {
		...
		if ( !$package->is_type( 'candidate_package' ) ) {
			return false;
		}

		$args = apply_filters( 'wp_job_board_wc_paid_listings_create_user_candidate_package_data', array(
			'post_title' => $package->get_title(),
			'post_status' => 'publish',
			'post_type' => 'job_package',
		), $user_id, $product_id, $order_id);
		...
		if ( $user_package_id ) {
			...
			do_action('wp_job_board_wc_paid_listings_create_user_candidate_package_meta', $user_package_id, $user_id, $product_id, $order_id);
		}
	...
	public static function create_user_resume_package( $user_id, $product_id, $order_id ) {
		...
		if ( !$package->is_type( 'resume_package' ) ) {
			return false;
		}

		$args = apply_filters( 'wp_job_board_wc_paid_listings_create_user_resume_package_data', array(
			'post_title' => $package->get_title(),
			'post_status' => 'publish',
			'post_type' => 'job_package',
		), $user_id, $product_id, $order_id);
		...
		if ( $user_package_id ) {
			...
			do_action('wp_job_board_wc_paid_listings_create_user_resume_package_meta', $user_package_id, $user_id, $product_id, $order_id);
		}
	...
	public static function increase_expiry_with_package( $user_id, $user_package_id ) {
		...
		if ( self::resume_package_is_valid( $user_id, $user_package_id ) && WP_Job_Board_User::is_candidate($user_id) ) {
			...
			if ( $post_type === 'candidate' ) {
				delete_post_meta( $candidate_id, WP_JOB_BOARD_CANDIDATE_PREFIX.'expiry_date' );

				update_post_meta( $candidate_id, '_user_package_id', $user_package_id );
				update_post_meta( $candidate_id, WP_JOB_BOARD_CANDIDATE_PREFIX.'package_duration', $resumes_duration );
				if ( $urgent_resumes  == 'on' ) {
					update_post_meta( $candidate_id, WP_JOB_BOARD_CANDIDATE_PREFIX.'urgent', 'on' );
					$_POST[WP_JOB_BOARD_CANDIDATE_PREFIX . 'urgent'] = 'on';
				} else {
					update_post_meta( $candidate_id, WP_JOB_BOARD_CANDIDATE_PREFIX.'urgent', '' );
				}

				if ( $featured_resumes == 'on' ) {
					update_post_meta( $candidate_id, WP_JOB_BOARD_CANDIDATE_PREFIX.'featured', 'on' );
					$_POST[WP_JOB_BOARD_CANDIDATE_PREFIX . 'featured'] = 'on';
				} else {
					update_post_meta( $candidate_id, WP_JOB_BOARD_CANDIDATE_PREFIX.'featured', '' );
				}
