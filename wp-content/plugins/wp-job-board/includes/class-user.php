...
class WP_Job_Board_User {
	...
	public static function role_caps() {
	    if ( current_user_can('wp_job_board_candidate') ) {
	        $subscriber = get_role('wp_job_board_candidate');
	    ...
	    if ( current_user_can('wp_job_board_employer') ) {
		    $subscriber = get_role('wp_job_board_employer');
		    $subscriber->add_cap('upload_files');
		    $subscriber->add_cap('edit_post');
		    $subscriber->add_cap('edit_published_pages');
		    $subscriber->add_cap('edit_others_pages');
	...
	public static function is_employer($user_id = null) {
	    ...
	    if ($employer_id > 0) {
	        ...
	        $post_employer = get_post($employer_id);
	        if ($post_employer && isset($post_employer->ID)) {
	            return true;
	        }
	    }
	    return false;
	...
	public static function is_candidate($user_id = 0) {
	    ...
	    if ($candidate_id > 0) {
	        ...
	        $post_candidate = get_post($candidate_id);
	        if ($post_candidate && isset($post_candidate->ID)) {
	            return true;
	        }
	    }
	    return false;
	...
	public static function process_forgot_password() {
		...
		if (empty ($error)) {
	        ...
	        if ( $user_login_auth == 'pending' && isset($user_obj->ID) ) {
	            echo json_encode(array(
	            	'status' => false,
	            	'message' => self::login_msg($user_obj)
	            ));
	            die();
	        } elseif ( $user_login_auth == 'denied' && isset($user_obj->ID) ) {
	            echo json_encode(array(
	            	'status' => false,
	            	'message' => __('Your account denied.', 'wp-job-board')
	            ));
	            die();
	        }
			...
			if( $update_user ) {
				...
				$to = $user->user_email;
				$subject = esc_html__( 'Your new password', 'wp-job-board' );
				$sender = 'From: '.get_option('name').' <'.$from.'>' . "\r\n";
				
				$message = esc_html__( 'Your new password is: ', 'wp-job-board' ) .$random_password;
	...
	public static function process_resend_approve_account() {
        ...
        if ( !empty($user_obj->ID) ) {
	        ...
	        if ( $user_login_auth == 'pending' ) {
				...
				// send the mail
				$result = wp_mail( $user_email, $subject, $content, $headers );
				if ( $result ) {
					echo json_encode(array(
		            	'status' => true,
