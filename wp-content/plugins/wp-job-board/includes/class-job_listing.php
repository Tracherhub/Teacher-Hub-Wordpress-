	...
	public static function display_apply_job_btn( $post_id = null ) {
		...
		if ( empty($application_deadline_date) || strtotime($application_deadline_date) >= strtotime('now') ) {
			...
			if ( $apply_type == 'external' ) {
				...
				if ( !empty($apply_url) ) {
					?>
					<a href="<?php echo esc_url($apply_url); ?>" class="btn btn-apply btn-apply-job-external"><?php esc_html_e('Apply Now', 'wp-job-board'); ?><i class="next flaticon-right-arrow"></i></a>
					<?php
				}
