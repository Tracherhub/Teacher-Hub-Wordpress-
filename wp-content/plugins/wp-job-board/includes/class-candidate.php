...
class WP_Job_Board_Candidate {
	...
	public static function process_apply_email() {
		...
		$result = wp_mail( $author_email, $email_subject, $email_content, $headers );
		if ( $result ) {
	...
	public static function process_remove_applied() {
		...
		$is_allowed = WP_Job_Board_Mixes::is_allowed_to_remove( get_current_user_id(), $applicant_id );
		$job_id = get_post_meta($applicant_id, WP_JOB_BOARD_APPLICANT_PREFIX . 'job_id', true);
		$is_allowed_job = WP_Job_Board_Mixes::is_allowed_to_remove( get_current_user_id(), $job_id );
