...
class WP_Job_Board_Applicant {
	...
	public static function init() {
		...
		add_action( 'wp_ajax_nopriv_wp_job_board_ajax_applicants_pagination',  array(__CLASS__, 'process_applicants_pagination') );

		// applicants reject
		add_action( 'wp_ajax_wp_job_board_ajax_reject_applied',  array(__CLASS__, 'process_applicant_reject') );
		add_action( 'wp_ajax_nopriv_wp_job_board_ajax_reject_applied',  array(__CLASS__, 'process_applicant_reject') );
		...
		add_action( 'wp_ajax_nopriv_wp_job_board_ajax_undo_reject_applied',  array(__CLASS__, 'process_undo_applicant_reject') );
		...
		add_action( 'wp_ajax_nopriv_wp_job_board_ajax_show_rejected_applicants',  array(__CLASS__, 'show_rejected_applicants') );
	...
	public static function process_applicant_reject() {
		...
		// return 
		$return = array( 'status' => true, 'msg' => esc_html__('Reject this applicant successful.', 'wp-job-board') );
	   	echo wp_json_encode($return);
	   	exit;
	}
	...
	public static function process_undo_applicant_reject() {
		...
		// return 
		$return = array( 'status' => true, 'msg' => esc_html__('Undo Reject this applicant successful.', 'wp-job-board') );
	   	echo wp_json_encode($return);
	   	exit;
	}
	...
	public static function show_applicants() {
		...
		$author_id = get_post_field ('post_author', $job_id);
		if ( $author_id != get_current_user_id() ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Can not to show applicants.', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
